...
app.get("*", function (req, res, next) {
  ...
  promise.then(function (data) {
    ...
    //staticRouter --- the location will not change in the server
    );
    res.send("\n      <!DOCTYPE html>\n      <html>\n        <head>\n          <title>SSR with RR</title>\n          <script src='/bundle.js' defer> </script> \n          <script>window.__INITIAL_DATA__=" + (0, _serializeJavascript2.default)(data) + "</script>\n        </head>\n        \n        <body>\n          <div id=\"app\">" + markup + "</div>\n        </body>\n      </html>\n      ");
  }).catch(next);
});
//inside of the html the script tag <script src="/bundle.js"></script> is telling the
// browser where to download the code; defer so we don't block any rendering

//<script>window.__INITIAL_DATA__=${serialize(data)}</script> so we get the same data (from server) down to the client
//

//after the initial render we want to download bundle.js so that react can take over
